...
const generateBody = (options, lines) => {
...
const validateHeader = (context, node, options) => {
    ...
    if (!comments.length) {
        context.report({
            ...
            fix(fixer) {
                return fixer
                    .insertTextBefore(node, wrapComment(options.comment.prefer, options, injectTemplateArgs(generateBody(options, options.header))) + '\n');
            }
        });
        return;
    }
